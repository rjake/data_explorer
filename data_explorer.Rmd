...
renderPlot({
    ...
    if (get_geom() == 'geom_point') p <- p + aes_(size = 2) + guides(size = F) + scale_size(range = c(2, 2))
    if (get_geom() == 'geom_count') p <- p + scale_size(range = c(2, 6))
...
output$gg_call <- 
    ...
    renderText({
        ...
        if(input$show_x_axis) {
            x_axis  <- 'axis.ticks.x = element_blank(), axis.text.x = element_blank()'
        }
        if (input$show_y_axis) {
            y_axis  <- 'axis.ticks.y = element_blank(), axis.text.y = element_blank()'
        }

        if(input$show_x_axis && input$show_y_axis){
            get_theme <- '+<br>theme(axis.ticks = element_blank(), axis.text = element_blank())'
        } else if(input$show_x_axis){
            get_theme <- paste0('+<br>theme(', x_axis, ")")
        } else if(input$show_y_axis){
            get_theme <- paste0('+<br>theme(', y_axis, ")")
        }
        ...
        HTML( 
           paste0('ggplot(',
              ...
              ifelse(facets() != '. ~ .',
                     paste0('+ <br>facet_grid(', facets(), ', scales = "free", space = "free")'), ''),
